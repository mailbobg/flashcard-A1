...
- 字母表 30 个（含 ä ö ü ß），标注**字母名的读音**——口语 Teil 1 要求拼写自己的名字，字母名本身就是考点。**点字母听字母名，点例词听单词**
...
- 词条全部来自官方 `A1_SD1_Wortliste`，**性别、复数、例句都是官方原文**，中文释义为手工编写
- **按官方 Themen 归类**（词表第 4 页的十个主题），复习范围和考纲对得上。
  介词、代词、情态动词这类不属于任何主题的词另立两个桶，界面上标明不是官方主题
...
- 首页有今日到期复习、按主题进度、难词本
- 词表可搜德语词也可搜中文，「遮住中文」用于看德语想意思的自测
- 进度存 `localStorage`，离线保留

**专项练习 · 考试必练**

首页入口，五个专项全部由 TTS 随机出题、即时判分：

- **数字 · 0-100**：听德语报数写阿拉伯数字，专治 `einundzwanzig`（=21，个位+und+十位）倒读；
  也可反着看数字写德语拼写
- **时间 · halb 的陷阱**：`halb drei` 是 2:30 不是 3:30。TTS 报时 → 选钟面，含 Viertel nach / vor
- **性别 · der / die / das**：看名词选冠词，选错显示词尾规律（`-ung/-heit/-keit`→die、
  `-er/-ling`→der、`-chen/-lein`→das，约八成可猜）
- **句子 · Satzklammer 语序**：句框可视化讲解 + 打乱词块排语序，覆盖情态动词 / 可分动词 / Perfekt
  三种触发情形
...
- **预生成音频 Seraphina**（默认）：德语 neural 音质，构建期合成好当静态文件发，
  运行时不依赖任何接口，播过一次浏览器就缓存。这也顺带绕开了上面那条限制——
  手机上原本只有 Anna，现在有个好音色可用，且离线也能听。
  每个音色约 25 MB，所以只随仓库带一个；想换或多带，用 `make_audio.py -v <音色>` 生成
- **谷歌在线**，每次朗读都要联网
